...
public class MatchEngine {
    ...
    static final class Holder {
        static final MatchEngine mInstance = new MatchEngine();
    }
    ...
    public static final float DEFAULT_VERIFYING_THRESHOLD = 4000f;
    ...
    private float mVerfyingThreshold;
    private opencv_core.CvMat mEarProjectBase;
    private List<SubjectBean> mData = new ArrayList<>();

    public void init() {
        mVerfyingThreshold = DEFAULT_VERIFYING_THRESHOLD;
        loadBase();
        update();
    }

    public float getThreshold() {
        return mVerfyingThreshold;
    }
    public void setThreshold(float threshold) {
        mVerfyingThreshold = threshold;
    }
    ...
    private void loadBase() {
        String filepath = FileEnvironment.baseAppOutputPath + FileEnvironment.PCA_2D_BASE_SETTING_FILENAME;
...
//        Log.e("channer test", "channer test " + filepath);
    ...
    private List<CvMat> loadOneImageSet(File file) {
        ...
        for (int i = 0; i < childFiles.length; i++) {
            if (childFiles[i].isFile() && childFiles[i].getName().indexOf(".jpg") > 1) {
